...
void ads_parse_read_buffer(uint8_t *buffer) {
  if (buffer[0] == ADS_SAMPLE) {
    ...
    Serial.print("ADS returned something Unexpected");
  }
}
...
int ads_read_polled(float *sample) {
  ...
  if (ret_val == ADS_OK) {
    ...
    if (buffer[0] != ADS_SAMPLE) {
      Serial.print("Unexpected　value in the buffer!");
    }
    if (buffer[0] == ADS_SAMPLE) {
...
/**
 * @brief Places ADS in poll mode. Each time sensor data is read a new sample is
 * taken
 *
 ...
 */
int ads_select_device(uint8_t device) { return ads_hal_select_device(device); }
...
int ads_init(ads_init_t *ads_init) {
  ...
  ads_hal_init(&ads_parse_read_buffer, ads_init->reset_pin,
               ads_init->datardy_pin, ads_init->xfer_size);
 ...
 *				ADS_CALIBRATE_SECOND can be at 45 - 255 degrees,
 ...
 */
int ads_calibrate(ADS_CALIBRATION_STEP_T ads_calibration_step,
                  uint8_t degrees) {
  uint8_t buffer[ADS_TRANSFER_SIZE];
 ...
 *				ADS_AXIS_0_EN enables axis zero and disables
 *axis one ADS_AXIS_1_EN enables axis one and disables axis zero
 *
...
/**
 * @brief Checks that the device id is ADS_ONE_AXIS. ADS should not be in free
...
void ads_set_xfer_size(uint8_t xfer_size) { m_ads.xfer_size = xfer_size; }
