...
int ads_multi_run(bool run) {
  ...
  if (pair_int[1]) {
    attachInterrupt(digitalPinToInterrupt(pair_int[1]), pair_two_interrupt,
                    FALLING);
  }
  if (pair_int[2]) {
    attachInterrupt(digitalPinToInterrupt(pair_int[2]), pair_three_interrupt,
                    FALLING);
  }
  if (pair_int[3]) {
    attachInterrupt(digitalPinToInterrupt(pair_int[3]), pair_four_interrupt,
                    FALLING);
  }

  if (pair_int[4]) {
    attachInterrupt(digitalPinToInterrupt(pair_int[4]), pair_five_interrupt,
                    FALLING);
  }
  ...
  if (_p[1].en) {
    ...
    if (_p[1].ads_dev_id == ADS_ONE_AXIS) {
      ...
      if (ads_hal_read_buffer(read_buffer, 5) == ADS_OK) {
        ...
        // sample[2] = (float)temp / 32.0f;
        temp = ads_int16_decode(&read_buffer[3]);
        _p[1].ang[1] = (float)temp / 32.0f;
        // sample[3] = (float)temp / 32.0f;
      }
    }
  }
