...
static uint8_t ads_transfer_size = ADS_TRANSFER_SIZE;

static uint32_t ADS_RESET_PIN = 0;
static uint32_t ADS_INTERRUPT_PIN = 0;

static uint8_t _address = ADS_DEFAULT_ONE_AXIS_ADDR;
...
int ads_hal_read_buffer(uint8_t *buffer, uint8_t len) {
  ...
  uint8_t i = 0;
  // delay(100); // this works for Sample frequency of 10 Hz :D
...
int ads_hal_init(void (*callback)(uint8_t *), uint32_t reset_pin,
                 uint32_t datardy_pin, uint8_t xfer_size) {
 ...
 */
void ads_hal_set_xfer_size(uint8_t xfer_size) { ads_transfer_size = xfer_size; }
