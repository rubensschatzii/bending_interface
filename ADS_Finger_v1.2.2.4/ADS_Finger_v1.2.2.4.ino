...
// Version 1.2.2.1 - slight modification to always sample data on a stable
...
#define loff 25
#define DPconst                                                                \
  0.88 // Constant between DIP and PIP joints. Literature points to values
...
// Function to get offset to adjust the initial measured angles to zero. UPDATE:
// the "offsetmean" function does not work properly, I was too dumb when I made
// it. Because only the latest angle is used as the input to the function, the
// output will always be equal to the input value. As I am lazy to change it,
// and it is not really important, I just keep it anyways.
float offsetmean(float angleoffset) {
  float ofsts[5];
  int i;
  for (i = 0; i < 5; i++) {
...
void getOffset(void) {
  ...
  // th3off = (temp2 - temp1) / (1 + (1 / DPconst));
  // th1off = temp1;
  ...
  // thbaseoff = offsetmean(p[0].ang[0]);
...
void ForwardInverse(void) {
  ...
  // th1 = p[1].ang[1] - th1off;
  // th2 = p[0].ang[1] - th1 - th2off;
  // th3 = th2 * DPconst - th3off;

  th1 = angle[3] - th1off;
  th3 = ((angle[1] - th3off) - (angle[3] - th1off)) / (1 + (1 / DPconst));
  th2 = (th3 / DPconst);
  // thb = angle[0] - thbaseoff;
  thb = angle[2] - thbaseoff;
  thbdiff = angle[0] - thbdiffoff;
  // th1 = angle[3] - th1off;
  // // th2 = (angle[1] - th1) / (1 + DPconst) - th2off;
  // th2 = (angle[1] - th3off) / (1 + DPconst);
  // th3 = angle[1] - th3off;
  // thb = angle[0] - thbaseoff;
  // Foward Kinematics
  ...
  // please, ignore the variables xpoff yoff etc..
...
void setup() {
  ...
  delay(100);
  // The program will only start after pressing the button attached in the PIN
  // A1. Or when A1 goes from HIGH to LOW.
  while (1) {
    int buttonstate;
...
void loop() {
  ...
  // waittime = looptime - runtime - previousruntime;
  // while (millis() - starttime < waittime + runtime)
  //   ;
  if (dataOnOff == 1) {
    logData();
  }
